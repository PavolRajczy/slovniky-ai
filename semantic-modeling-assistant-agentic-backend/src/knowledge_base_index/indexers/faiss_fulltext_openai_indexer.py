...
class FAISSFullTextOpenAIKnowledgeDocumentIndexer(KnowledgeDocumentIndexer):
    """
    ...
    over knowledge document content using OpenAI's text-embedding-3-small model.
    """
    ...
    def __init__(self,
        ...
        """
        Initialize the FAISS-based knowledge document indexer.
        ...
        # Initialize chunker
        self.chunker = SemchunkIndexDocumentChunker(chunk_size=chunk_size, chunk_overlap=0, target_model=target_model)
    ...
    def build_index(self, knowledge_document: KnowledgeDocument) -> None:
        ...
        """
        print(f"Building index for knowledge document: {knowledge_document.id}")
        ...
        print(f"Successfully built index with {len(self.chunk_metadata)} chunks")
    ...
    def build_or_load_index(self, knowledge_document: KnowledgeDocument) -> None:
        ...
        if self.load_index(knowledge_document):
            print("Loaded existing index from cache")
            return
        ...
        try:
            ...
            print(f"Warning: Failed to store index: {e}")
    ...
    def store_index(self, knowledge_document: KnowledgeDocument) -> None:
        ...
        index_dir = cache_path.parent / "indexes" / "faiss_fulltext_openai"
        index_dir.mkdir(parents=True, exist_ok=True)
        
        print(f"Storing index to: {index_dir}")
        ...
        print(f"Successfully stored index with {len(self.chunk_metadata)} chunks")
    ...
    def load_index(self, knowledge_document: KnowledgeDocument) -> bool:
        ...
        """
        if not knowledge_document.cacheFilePath:
            print("Knowledge document has no cacheFilePath, cannot load index")
            return False
        ...
        for file_path in required_files:
            if not file_path.exists():
                print(f"Required index file missing: {file_path}")
                return False
        ...
        try:
            ...
            print(f"Successfully loaded index with {len(self.chunk_metadata)} chunks")
            return True
    ...
    def _create_index_documents(self, knowledge_document: KnowledgeDocument) -> List[IndexDocument]:
        ...
        It creates an IndexDocument for each element that has a content.
        ...
        def process_element(element: KnowledgeDocumentElement, parent_content: str = ""):
            ...
            stripped_element_content = element.content.strip() if element.content else ""

            # Create index document for this element
            if stripped_element_content:
                index_doc = IndexDocument(
                    id=str(uuid.uuid4()),
                    element_id=element.id,
                    content=stripped_element_content,
